...
public class ExportDataDialogFragment extends BaseDialogFragment implements ExportDataContract.View {
    ...
    TextInputEditText exportDataStartDateText;
    @BindView(R.id.export_data_end_date)
    TextInputEditText exportDataEndDateText;
    ...
    private Date exportStartDate = new Date();
    private Date exportEndDate = new Date();
    ...
    private Boolean isTimeFrameCorrect() {
        Calendar c = Calendar.getInstance();
        if (exportStartDate.getTime() > exportEndDate.getTime()) {
            showError("The TimeFrame is not valid.");
            return false;
        } else if (exportStartDate.getTime() > c.getTimeInMillis()) {
            showError("The Start Date is not valid.");
            return false;
        }
    ...
    private void setupUI(View dialog_view) {
        ...
        exportDataStartDateText.setKeyListener(null);
        exportDataStartDateText.setOnClickListener(v -> {
            isStartPicker = true;
            setDatePicker();
        });

        exportDataEndDateText.setKeyListener(null);
    ...
    @Override
    public void exportToCSV(List<CashTransaction> cashTransactions) {
        // Go Through All The Transactions in The DB And Export Them As CSV
        Timber.i("CSV Exporting");
        ...
        if (dirIsCreated) {
            String fileName = String.format("transactions_%s.csv",
                    AppUtils.formatDate(new Date(), MAIN_DATE_FORMAT));
            ...
            try {
                ...
                for (CashTransaction transaction : cashTransactions) {
                    if (checkTransactionTime(transaction)) {
                ...
                shareFile(csvFile);
                showMessage("The Exported File has been saved in /CashNoteFiles folder");
    ...
    private boolean checkTransactionTime(CashTransaction transaction) {
        if (transaction.getLastUpdatedDate() <= exportStartDate.getTime()
                && transaction.getLastUpdatedDate() >= exportStartDate.getTime()) {
            return true;
        } else {
            return false;
        }
